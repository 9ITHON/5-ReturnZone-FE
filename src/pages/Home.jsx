...
const Home = () => {
  ...
  if (loading) {
    return (
      <div className="relative w-[390px] h-[844px] bg-white flex flex-col items-center mx-auto overflow-hidden">
        <Header />
        <FilterBar onFilter={handleFilter} selectedLocation={selectedLocation} onLocationButton={handleLocationButton} selectedFilters={selectedFilters} />
  ...
  return (
    <div className="relative w-[390px] h-[844px] bg-white flex flex-col items-center mx-auto overflow-hidden">
      <LocationPermission 
        onRequestPermission={() => filterItems(selectedCategory, selectedLocation, selectedFilters[0])}
      ...
      {categoryOpen && (
        <div className="fixed inset-0 z-30 flex items-end justify-center w-full max-w-[390px] mx-auto bg-black/20 backdrop-blur-sm">
          <div className="w-full max-w-[390px] bg-white rounded-t-xl z-40">
            ...
            <CategoryFilter selectedCategory={selectedCategory} onCategorySelect={(cat) => { handleCategorySelect(cat); setCategoryOpen(false); }} />
          </div>
        </div>
      )}
      ...
      <BottomNav />
      <LocationMapModal open={mapOpen} onClose={() => setMapOpen(false)} onSelect={handleMapSelect} />
    </div>
  )
}
